...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    """
    back_point = list(vals[:])
    back_point[arg] = vals[arg] - epsilon
    
    forward_point = list(vals[:])
    forward_point[arg] = vals[arg] + epsilon

    return (f(*forward_point) - f(*back_point)) / (2 * epsilon)
...
class ScalarFunction(FunctionBase):
    ...
    @staticmethod
    def backward(ctx: Context, d_out):
        r"""
...
class Sigmoid(ScalarFunction):
    ...
    @staticmethod
    def backward(ctx: Context, d_output):
        sigma = operators.sigmoid(ctx.saved_values())
